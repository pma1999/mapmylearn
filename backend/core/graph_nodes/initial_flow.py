...
async def create_learning_path(state: LearningPathState) -> Dict[str, Any]:
    ...
    # Spawn curiosity generation in parallel if not started and we have results
    try:
        if search_service_results and not state.get("curiosities_generation_started"):
            state["curiosities_generation_started"] = True
            asyncio.create_task(_generate_curiosity_items_parallel(state, search_service_results))
    except Exception:
...
async def _generate_curiosity_items_parallel(state: LearningPathState, search_service_results: List[SearchServiceResult]):
    ...
    try:
        ...
        # Desired count bounds
        desired_count_env = os.environ.get("CURIOSITY_COUNT", "14")
        try:
            desired_count = max(8, min(16, int(desired_count_env)))
        except ValueError:
            desired_count = 14

        # Prepare prompt template and params
        prompt = ChatPromptTemplate.from_template(CURIOSITY_TIPS_PROMPT)
        params = {
        ...
        if progress_callback and items:
            payload_items = [{"text": it.text, "category": it.category} for it in items if getattr(it, 'text', None)]
            if payload_items:
                await progress_callback(
                    f"Found {len(payload_items)} curated insights about '{state['user_topic']}'.",
                    phase="curiosities",
                    phase_progress=1.0,
                    preview_data={
                        "type": "curiosities_ready",
            ...
            await progress_callback(
                "No additional curiosities available at this time.",
                phase="curiosities",
                action="completed"
            )
    except Exception as e:
        logger.exception(f"Curiosity generation failed: {e}")
